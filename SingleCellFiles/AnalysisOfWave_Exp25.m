...
%%  File for analysis of Long period data (Exp25) for Gould, Domijan et al.
...
addpath([pwd '/subplot_tight/']) %this is the additional toolbox.
%from: https://uk.mathworks.com/matlabcentral/fileexchange/30884-controllable-tight-subplot

%should not need these:
%addpath([pwd 'TESTperiod_results/')
...
[aa, texts]=xlsread('Data_singlecell/Most_recent_data/Exp25_final_coordinates/interpolated/Individual results_1h.xlsx');
aa(1:4, :)=[];
texts(1:4, :)=[];
...
posi=12:18;
for j=1:length(posi)
xs=csvread(['Data_singlecell/Most_recent_data/Exp25_final_coordinates/interpolated/interpolateddata_Exp25_pos' num2str(posi(j)) '_spacing1h.csv']);
...
[aa2, texts2]=xlsread('Data_singlecell/Most_recent_data/Exp25_final_coordinates/interpolated/metamorph_EXP25_1hr_interp.xlsx');
...
%     clear xa
        ...
        posfile=dir([pwd , '/Data_singlecell/Most_recent_data/' name '_final_coordinates/',['pos_' name2 '*tracked.csv']]);
...
%  end
   ...
   elseif i==17
    ...
    if i==12
        ...
        pos{i}.YFP( :, [37]) =[]; %lloks like int he interpolation file we left out this trace!!
        pos{i}.x( :, [37]) =[];
        pos{i}.y( :, [37]) =[];
        pos{i}.z( :, [37]) =[];
...
%   end
  ...
  if i==18
      pos{i}.YFP(:, 272)=[];   %cell 1000000271 i missiing
  end
...
% time points 
if strcmp(name, 'Exp25')==1
[Times,txt2]=xlsread(['timestamp_Exp25.xlsx'], 'B8:AI26');
pos{i}.time= Times(i,:);
end
...
%for k=3% start_pos: numpos %  k=start_pos: numpos for posi not posix
%     i=posi(k); %posix(k);
%     xxs=[ xxs pos{i}.xmod];
%     yys=[ yys pos{i}.ymod];
...
%     pergofsmF=[pergofs; pos{i}.periodsgofs(:,2)];
%     perignoremF=[perignore; pos{i}.periodsignore(:,2)];
%      persSR=[persSR; pos{i}.periods(:,3)];
%     pergofsSR=[pergofs; pos{i}.periodsgofs(:,3)];
%     perignoreSR=[perignore; pos{i}.periodsignore(:,3)];
...
for k=1: numpos %for posi not posix
    ...
    times=[times repmat(pos{i}.time',1,(size(pos{i}.YFP,2)))];
    elseif k==2
      % keep=find(pos{i}.ymod(end, :) >583.65);   
    ...
    elseif k==3
         % keep=find(pos{i}.ymod(end, :) >923.63);   
    ...
    count(k)=size(pos{i}.YFP,2);
       countkeep(k)=size(pos{i}.YFP(:,keep),2);
    elseif k==4
        %  keep=find(pos{i}.xmod(end, :) > 404.9700);   
            %  dontkeep=find(pos{i}.ymod(end, :) <max(max(pos{14}.ymod)) & pos{i}.xmod(end, :) <max(max(pos{14}.xmod)));  %how originally done  
...
%save('Exp25_analysed_total_Dec18', 'countkeep', 'traces', 'times', 'xxs_adjusted', 'yys_adjusted', 'zzs_adjusted', 'pers', 'persSR', 'persmF', 'perignore' ,'perignoreSR', 'perignoremF','pergofs' , 'pergofsSR', 'pergofsmF')  
...
print('-Painters', kkq, 'New_FigureSI3D_exp25','-dpdf','-r300')
...
print('-Painters', kkq, 'New_FigureSI3C_exp25','-dpdf','-r300')
%% Figure2 SI2 E:
clf
kkq=figure;
set(kkq,'PaperUnits', 'centimeters',  'PaperPosition', [0 0  3000 2400]/300)
...
colormap(jet(128));
caxis([18 35]);
h = colorbar( 'Location', 'north', 'Direction', 'reverse', 'XTick',[18 35],'TickLabels',{'35h' '18h'}, 'FontSize',7, 'Position',[[0.65 0.2 0.2 0.01]]);
...
print('-Painters', kkq, 'New_FigureSI3E_exp25','-dpdf','-r300')
keyboard
...
set(gca, 'LineWidth', 1.2)
set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
print('-Painters', kkq, 'New_FigureSI3F_exp25','-dpdf','-r300')
%% Figure2 SI2 H: y positive vs. z periods. 
clf
kkq=figure;
set(kkq,'PaperUnits', 'centimeters',  'PaperPosition', [0 0  1700 2400]/300)
cc=[];
...
print('-Painters', kkq, 'New_FigureSI_PeriodZdirectionALL_exp25','-dpdf','-r300')
%% Figure2 SI2 J: y positive vs. z periods (only roots)
clf
kkq=figure;
set(kkq,'PaperUnits', 'centimeters',  'PaperPosition', [0 0  1700 2400]/300)
%set(kkq,'PaperUnits', 'centimeters',  'PaperPosition', [0 0  800 2400]/300)
cc=[];
...
ylim([indJ1(1) 1300]) %all three lines removed MD2018
set(gca,'Ytick',1000:500:10500,'YTickLabel',(1:0.5:10.5)) %all three lines removed MD2018
set(gca,'Ytick',indJ1(1):500:10500,'YTickLabel',(0:0.5:10.5)) %MD 2018.
set(gca,'Xtick',0:50:10500,'XTickLabel',(0:0.05:10.5)) %MD 2018.
set(gca,'Fontsize', 7)
...
print('-Painters', kkq, 'New_FigureSI_PeriodZdirection_exp25','-dpdf','-r300')
...
times=times(:, reloc); 
%% Info for phases if you want to plot a phase plot.
mina=3000; maxa=0;
...
set(gcf, 'Color', 'black')
 func_hndl{2}(1,17,ti, 0.001); %-60);
   for i=1: size(xxs,2)
              set(gcf, 'Color', 'k')
  %  plot(xxs(ti,i)',yys(ti,i)', '.','Markersize',full6(ti,i)/2.4+6,'Color',(dd6(ti,i,1:3)))
   
   plot(xxs(ti,i)',yys(ti,i)', '.','Markersize',full6x(ti,i)/20+2,'Color',(dd6x(ti,i,1:3)))
...
set(gca,'xtick',[])
set(gca,'ytick',[])
     xlim([min(min(xxs)) max(max(xxs))])
     ylim([min(min(yys)) max(max(yys))])
hold on
...
print('-Painters',kkq, 'Filmstrip_root_exp25','-dpdf','-r300')

kkq=figure;
set(kkq,'PaperUnits', 'centimeters',  'PaperPosition', [0 0  5800 700]/300)
func_hndl={@subplot;@subplot_tight};
for  ti=8:1:34%61:1:120
       set(gcf, 'Color', 'black')
   % set(gca,'DataAspectRatio',[1 1 1])
 %  subplot_tight(5,7,ti, 0.1) 
  

set(gcf, 'Color', 'black')
 func_hndl{2}(1,17,ti-17, 0.003); %-60);
   for i=1: size(xxs,2)
              set(gcf, 'Color', 'k')
...
set(gcf,'color','w');
set(gcf, 'InvertHardCopy', 'off');
print('-Painters',kkq, 'Filmstrip_root_exp25_part2','-dpdf','-r300')
