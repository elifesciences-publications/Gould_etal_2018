%  File for analysis of WT data (Exp21) for Gould, Domijan et al.
...
[aa, texts]=xlsread('Data_singlecell/Most_recent_data/Exp21_final_coordinates/interpolated/Individual results_1h.xlsx');
aa(1:4, :)=[];
texts(1:4, :)=[];
...
posi=13:20;
for j=1:length(posi)
xs=csvread(['Data_singlecell/Most_recent_data/Exp21_final_coordinates/interpolated/interpolateddata_Exp21_pos' num2str(posi(j)) '_spacing1h.csv']);
...
if i==14 || i==19
        posfile=dir([pwd , '/Data_singlecell/Most_recent_data/' name '_final_coordinates/',['*_pos_' name2 '*tracked*.csv']]);
else
 ...
 if i==16
    ...
    YFPfile=dir([pwd , '/Data_singlecell/Most_recent_data/'  name '_final_coordinates/',['*mean*' name2 '*YFP_*.csv']]);
...
% this is for Exp 21, there are fewer time points for position 16 and
% above (experiment terminated there). 
% if i>=16 
%     pos{i}.time(end)=[];
% end
%  
...
for k=start_pos:numpos
    ...
    hold on
    plotcube([vals vals 100] , [indJ1(k) indJ2(k) 1], .3 ,'k')
end
...
%% if individual positions (do this!)
% for k=5% start_pos: numpos
%     i=posi(k);
...
% 
for k= 1 : numpos %:3 %  1 :numpos%2:5%start_pos% numpos
     i=posi(k);
 if k==1
    xxs=[ xxs pos{i}.xmod(1:30, :)];
    yys=[ yys pos{i}.ymod(1:30, :)];
...
%     for j=1: size(pos{i}.xmod(end, :), 2)
%       A=[pos{i}.xmod(end, j) pos{i}.ymod(end, j)]; 
%       B=[1 1]; 
%       yysproj(j)=(dot(A,B)/norm(B)^2); 
%       
%     end   
  elseif k==2
  ...
  %  keep=find(pos{i}.ymod(end, :) >724);    
   % keep=find(pos{i}.ymod(end, :) >0);
...
elseif k==5
  ...
  countkeep(k)=size(pos{i}.YFP(:,keep),2);
    ...
    xxs=[ xxs pos{i}.xmod(1:30, :)];
    yys=[ yys pos{i}.ymod(1:30, :)];
    zzs=[ zzs pos{i}.z(1:30, :)]; %MD 03.03.2017 : instead of 'keep'
    traces=[ traces pos{i}.YFP(1:30, :)];
    times=[times repmat(pos{i}.time(1:30)',1,(size(pos{i}.YFP,2)))];
    pers=[pers; pos{i}.periods(:,1)];
    pergofs=[pergofs; pos{i}.periodsgofs(:,1)];
...
%save('Exp21_analysed_total_Jan18', 'countkeep', 'traces', 'times', 'xxs_adjusted', 'yys_adjusted', 'zzs_adjusted', 'pers', 'persSR', 'persmF', 'perignore' ,'perignoreSR', 'perignoremF','pergofs' , 'pergofsSR', 'pergofsmF')  
keyboard
...
ylim([indJ1(1) 4200]) %all three lines removed MD2018
...
print('-Painters', kkq, 'New_FigureSI2D_exp21','-dpdf','-r300')
...
set(gca,'Ytick',1000:500:10500,'YTickLabel',(1:0.5:10.5)) %all three lines removed MD2018
set(gca,'Ytick',indJ1(1):500:10500,'YTickLabel',(0:0.5:10.5)) %MD 2018.
set(gca,'Xtick',0:500:6500,'XTickLabel',0:0.5:6.5) %all three lines removed MD2018
...
print('-Painters', kkq, 'New_FigureSI2C_exp21','-dpdf','-r300')
%% Figure SI 2E:
clf
kkq=figure;
set(kkq,'PaperUnits', 'centimeters',  'PaperPosition', [0 0  2000 3600]/300)
...
print('-Painters', kkq, 'New_FigureSI2E_exp21','-dpdf','-r300')
keyboard
...
colormap(jet(128));
caxis([18 35]);
%h = colorbar( 'Location', 'north', 'Direction', 'reverse', 'XTick',[18 35],'TickLabels',{'35h' '18h'}, 'FontSize',7, 'Position',[[0.4 0.032 0.2 0.01]]);
...
set(gca, 'LineWidth', 1.2)
set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
print('-Painters', kkq, 'New_FigureSI2F_exp21','-dpdf','-r300')
...
print('-Painters', kkq, 'New_FigureSI_PeriodZdirectionALL_exp21','-dpdf','-r300')
...
colormap(jet(128));
caxis([18 35]);
%h = colorbar( 'Location', 'north', 'Direction', 'reverse', 'XTick',[18 35],'TickLabels',{'35h' '18h'}, 'FontSize',7, 'Position', [[0.4 0.03 0.4 0.01]]);
set(gca, 'LineWidth', 1.2)
set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
print('-Painters', kkq, 'New_FigureSI_PeriodZdirection_exp21','-dpdf','-r300')
...
ha=HeatMap(tracesminrhythm'*2-1, 'colormap', 'bone');
addXLabel(ha, 'Time', 'FontSize', 25)
addYLabel(ha, 'Cell index', 'FontSize', 25)

ha=HeatMap(tracesminrhythma'*2-1, 'colormap', 'bone');
addXLabel(ha, 'Time', 'FontSize', 25)
addYLabel(ha, 'Cell index', 'FontSize', 25)
...
%heatmap of the Root tip-root &  root-hypocotyl sections
%heatmap where y-axis is cell label:
%heatmap where y-axis is position along the root:



%averaging over the same levels)
sections=30;
interval=(max(max(yys))-min(min(yys)))/sections;


for k=1:sections
    int1=min(min(yys))+interval*(k-1);
    int2=min(min(yys))+interval*k;
    [row,col]=find(yys>=int1 & yys<int2); 
    
    traces3=nan(size(traces2,1), size(traces2,2)); 
    traces3(row, col)=traces2(row, col); 
    
    xa=nansum(traces3,2);
    traces4(:,k)=nansum(traces3,2);
    traces4(find(xa==0),k)=nan;
    
    clear row col xa
end
%% SI2G- filmstrip of root.
func_hndl={@subplot;@subplot_tight};
kkq=figure;
set(kkq,'PaperUnits', 'centimeters',  'PaperPosition', [0 0  4800 700]/300)
for  ti=1:1:15%61:1:120
       set(gcf, 'Color', 'black')
 whitebg(1, 'k')
set(gcf, 'Color', 'black')
 func_hndl{2}(1,15,ti, 0.001); %-60);
   for i=1: sum(countkeep(1:5))
           set(gcf, 'Color', 'k')
           if  pergofs(i)<gof_cutoff && pergofsmF(i)<gof_cutoff && pergofsSR(i)<gof_cutoff2  && abs(pers(i)- persSR(i))<2.5 && abs(persmF(i)- persSR(i))<2.5 && abs(persmF(i)-pers(i))<2.5 && sum(perignoremF(i)+perignoreSR(i)+perignore(i))==0%<0.9  & pos{
...
print('-Painters',kkq, 'NewFilmstrip_root_exp21','-dpdf','-r300')
...
set(gcf,'color','w');
set(gcf, 'InvertHardCopy', 'off');
print('-Painters',kkq, 'NewFilmstrip_root_exp21_part2','-dpdf','-r300')
