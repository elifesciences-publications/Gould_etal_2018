%  File for analysis of WT data (Exp26) for Gould, Domijan et al.
...
clear all 
%%  
name='Exp26'; %Experiment to run. 
%% add in relevant paths to the data
addpath([pwd '/Data_singlecell/'])
addpath([pwd '/Data_singlecell/Most_recent_data/'])
addpath([pwd '/Data_singlecell/Most_recent_data/' name '_final_coordinates'])
...
[aa, texts]=xlsread('Data_singlecell/Most_recent_data/Exp26_final_coordinates/interpolated/Individual results.xlsx');
aa(1:4, :)=[];
texts(1:4, :)=[];
...
posi=1:6;
for j=1:length(posi)
xs=csvread(['Data_singlecell/Most_recent_data/Exp26_final_coordinates/interpolated/interpolateddata_Exp26_pos' num2str(posi(j)) '.csv']);
...
%     pos{i}.periods(xa(:,1),2)=xa(:,5);
    ...
    clear xa
     posfile=dir([pwd , '/Data_singlecell/Most_recent_data/' name '_final_coordinates/',[name2 '*tracked*.csv']]);
 ...
 % cell_labels is the labels of the cells 
    ...
    if i==4
    YFPfile=dir([pwd , '/Data_singlecell/Most_recent_data/'  name '_final_coordinates/',['mean*' name2 '*YFP_median*.csv']]);
    else
     YFPfile=dir([pwd ,'/Data_singlecell/Most_recent_data/'  name '_final_coordinates/',['*_' name2 '*YFP_median*.csv']]);
    end
...
% time points 
if strcmp(name, 'Exp26')==1
[Times,txt2]=xlsread(['timestamp.xlsx'], 'C8:DR15');
pos{i}.time= Times(i,:);
elseif strcmp(name, 'Exp21')==1
    
[Times,txt2]=xlsread(['timestamp2.xlsx'], 'B8:AG27');
...
eachpix=126/43; %this is how many microns per pixel
...
for k= start_pos:numpos
    i=posi(k);
    pos{i}.xmod=indJ2(k)-vals/2+ (pos{i}.x);%adjust vals MD dec2018
   ...
   elseif k==3 
    ...
    count(k)=size(pos{i}.YFP,2);
     countkeep(k)=size(pos{i}.YFP(:,keep),2);
      ...
      elseif k==5 
    %[ik,ja]=find(pos{i}.ymod(end, :) <max(max(pos{4}.ymod)) & pos{i}.xmod(end, :) <max(max(pos{4}.xmod)));   
...
%save('Exp26_analysed_total_Dec18', 'countkeep', 'traces', 'times', 'xxs_adjusted', 'yys_adjusted', 'zzs_adjusted', 'pers', 'persSR', 'persmF', 'perignore' ,'perignoreSR', 'perignoremF','pergofs' , 'pergofsSR', 'pergofsmF')  
keyboard
...
for k=1:size(pers,1)
     % only plot data which is treated as periodic (conditions outlined in  the paper)
      if   pergofs(k)<gof_cutoff && pergofsmF(k)<gof_cutoff && pergofsSR(k)<gof_cutoff2  && abs(pers(k)- persSR(k))<2.5 && abs(persmF(k)- persSR(k))<2.5 && abs(persmF(k)-pers(k))<2.5 && sum(perignoremF(k)+perignoreSR(k)+perignore(k))==0 && pers(k)<=35 && pers(k)>=18
        ...
        %col2=interp1([min(max(traces)) max(max(traces))],[1 siz],max(traces(:,k)));
...
if exist('New_Figure2G_exp26.pdf')~=2
print('-Painters', kkq, 'New_Figure2G_exp26','-dpdf','-r300')
end
...
for k=1:size(pers,1)
      if   pergofs(k)<gof_cutoff && pergofsmF(k)<gof_cutoff && pergofsSR(k)<gof_cutoff2  && abs(pers(k)- persSR(k))<2.5 && abs(persmF(k)- persSR(k))<2.5 && abs(persmF(k)-pers(k))<2.5 && sum(perignoremF(k)+perignoreSR(k)+perignore(k))==0 && pers(k)<=35 && pers(k)>=18
       % only plot data which is treated as periodic (conditions outlined in  the paper)
        col2=interp1([min(min(traces)) max(max(traces))],[1 siz],max(traces(:,k))); %orignal in paper
        %col2=interp1([min(max(traces)) max(max(traces))],[1 siz],max(traces(:,k)));
        colr= interp1(1:siz,flipud(cmap),col2);
        hold on
...
%% Figure 2G sizing and saving options
ylim([indJ1(1)-vals/2 2700]) %all three lines removed MD2018
set(gca,'Ytick',1000:500:10500,'YTickLabel',(1:0.5:10.5)) %all three lines removed MD2018
set(gca,'Ytick',indJ1(1)-vals/2:500:10500,'YTickLabel',(0:0.5:10.5)) %MD 2018.
% remove 
% query limits of x:
xlimits=xlim;
%set(gca,'Xtick',400:500:14500,'XTickLabel',(0:0.5:10.5))  % xlimit bottom %should be 400
...
%gc
if exist('New_Figure2F_exp26.pdf')~=2
print('-Painters', kkq, 'New_Figure2F_exp26','-dpdf','-r300')
end
%% Figure 3 A:
clf
kkq=figure;
...
set(gca,'Ytick',1000:500:10500,'YTickLabel',(1:0.5:10.5)) %all three lines removed MD2018
set(gca,'Ytick',indJ1(1)-vals/2:500:10500,'YTickLabel',(0:0.5:10.5)) %MD 2018. 
set(gca,'Xtick',400:500:14500,'XTickLabel',(0:0.5:10.5)) %all three lines removed MD2018
...
set(gca, 'LineWidth', 1.2)
set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
if exist('New_Figure3A_exp26.pdf')~=2
print('-Painters', kkq, 'New_Figure3A_exp26','-dpdf','-r300')
end
keyboard
...
set(gca, 'LineWidth', 1.2)
set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
if exist('New_Figure3B_exp26.pdf')~=2
print('-Painters', kkq, 'New_Figure3B_exp26','-dpdf','-r300')
end
...
set(gca,'Ytick',1000:500:10500,'YTickLabel',(1:0.5:10.5)) %all three lines removed MD2018
set(gca,'Ytick',indJ1(1)-vals/2:500:10500,'YTickLabel',(0:0.5:10.5)) %MD 2018.
set(gca,'Xtick',0:50:10500,'XTickLabel',(0:0.05:10.5)) %MD 2018.
set(gca,'Fontsize', 7)
ylabel('y (mm)','Fontsize', 7)
...
print('-Painters', kkq, 'New_Figure3C_exp26','-dpdf','-r300')
...
set(gca, 'LineWidth', 1.2)
set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
ylim([indJ1(1)-vals/2 1300]) %all three lines removed MD2018
set(gca,'Ytick',1000:500:10500,'YTickLabel',(1:0.5:10.5)) %all three lines removed MD2018
set(gca,'Ytick',indJ1(1)-vals/2:500:10500,'YTickLabel',(0:0.5:10.5)) %MD 2018.
set(gca,'Xtick',0:50:10500,'XTickLabel',(0:0.05:10.5)) %MD 2018.
set(gca,'Fontsize', 7)
...
set(gca, 'LineWidth', 1.2)
set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
if exist('New_Figure3D_exp26.pdf')~=2
print('-Painters', kkq, 'New_Figure3D_exp26','-dpdf','-r300')
end
...
siz=size(cmap,1);
colo=[ 1 1 0.2; 1 0.2 0.2; 0.4 0.8 0; 0.8 0.2 1; 0 1 1 ; 0.8 1 0.2; 1 0 0.8; 1 0.2 0.2];
figure(4)
colo=[ 0 0 0 ; 1 0.2 0.2; 0.4 0.8 0; 0.8 0.2 1; 0 1 1 ; 0.8 1 0.2; 1 0 0.8; 1 0.2 0.2];
...
if figs==1
   ...
   set(gca, 'LineWidth', 1.2)
   set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
   if exist('New_Figure3H_exp26.pdf')~=2
print('-Painters', kkq, 'New_Figure3H_exp26','-dpdf','-r300')
end
elseif figs==2
    xx=0:200:10500;% MD Jan2018
   set(gca,'YTickLabel', cellstr(num2str(xx(:)/1000, '%.2f')))% MD Jan201
   set(gca, 'LineWidth', 1.2)
    set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
    set(gca, 'LineWidth', 1.2)
    set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
    if exist('New_Figure3H_exp26_rescaled.pdf')~=2
    print('-Painters', kkq, 'New_Figure3G_exp26_rescaled','-dpdf','-r300')
    end
elseif figs==3
    xx=floor(min(min(yys(:, StartNum(figs):StartNum(figs+1)-1)))):200:10500;% MD Jan2018
    set(gca,'YTickLabel', cellstr(num2str(xx(:)/1000, '%.2f')))% MD Jan2018
    set(gca, 'LineWidth', 1.2)
    set(findall(gca, 'Type', 'Line'), 'LineWidth', 1.2);
    if exist('New_Figure3F_exp26.pdf')~=2
print('-Painters', kkq, 'New_Figure3F_exp26','-dpdf','-r300')
end
end   
end
...
if exist('NewFilmstrip_root_exp26.pdf')~=2
print('-Painters',kkq, 'NewFilmstrip_root_exp26','-dpdf','-r300')
end
...
for  ti= 16:1:34
   ...
   if  pergofs(i)<gof_cutoff && pergofsmF(i)<gof_cutoff && pergofsSR(i)<gof_cutoff2  && abs(pers(i)- persSR(i))<2.5 && abs(persmF(i)- persSR(i))<2.5 && abs(persmF(i)-pers(i))<2.5 && sum(perignoremF(i)+perignoreSR(i)+perignore(i))==0%<0.9  & pos{i}.periods(j,5)<=35 & pos{i}.periods(j,5)>=18 & pos{i}.periods(j,9)>0 & abs(pos{i}.periods(j,5)- pos{i}.periods(j,1))<=5% pos{i}.periods(j,2)<0.5
     plot(xxs_adjusted(ti,i)',yys_adjusted(ti,i)', '.','Markersize',full6x(ti,i)/20+2,'Color',(dd6x(ti,i,1:3))) %1.2
    %plot(xxs(ti,i)',yys(ti,i)', '.','Markersize',full6(ti,i)/2.5+12,'Color',(dd6(ti,i,1:3)))
    if ti==16
...
set(gcf,'color','w');
set(gcf, 'InvertHardCopy', 'off');
if exist('NewFilmstrip_lowerhyp_exp26.pdf')~=2
print('-Painters',kkq, 'NewFilmstrip_lowerhyp_exp26','-dpdf','-r300')
end
...
set(gcf,'color','w');
set(gcf, 'InvertHardCopy', 'off');
if exist('NewFilmstrip_upperhyp_exp26.pdf')~=2
print('-Painters',kkq, 'NewFilmstrip_upperhyp_exp26','-dpdf','-r300')
end
...
set(gcf,'color','w');
set(gcf, 'InvertHardCopy', 'off');
if exist('NewFilmstrip_cot_exp26.pdf')~=2
print('-Painters',kkq, 'NewFilmstrip_cot_exp26','-dpdf','-r300')
end
